    ...
    const double prune_ratio =
        total == 0 ? 0.0
                   : static_cast<double>(avg_stats.periodic_skipped +
                         ...
                         total;
    out << SINGLE_ROUND_IMPL_NAME << ',' << bits << ',' << p << ',' << k
        << ',' << w << ',' << M << ",3," << op << ',' << trials << ','
        << warmup << ',' << timing.mean << ',' << timing.median << ','
        << timing.p95 << ',' << correctness_failures << ',' << failure_class
        << ',' << prune_ratio << ',' << avg_stats.periodic_skipped << ','
        << avg_stats.zero_skipped << ',' << avg_stats.executed_terms << "\n";
}
